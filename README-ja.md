...
このスクリプトは、内部で以下のdocker runコマンドを呼び出します。<br>
...
Intel Linux環境およびARM macOS環境において動作することを確認済みです。

以下を実行することで、Debianの公式パッケージとして供給されているオープンソースのWEBブラウザーコンポーネントであるChromiumを用いてコンテナイメージが作成されます。
...
>
> また、引用やコードブロックの背景がフロート化した画像の下に溢れないように、`blockquote`や`pre`も`figure`の中に入るようになりました。
> **(Ver.0.13で仕様変更)**
>
...
引用ブロック(行頭に`> `を置いたブロック)中のどこかに`<!-- left -->`または`<!-- right -->`のようなHTMLコメントを記述すると、そのブロックはページの左端または右端にフロート配置され、続く文書が回り込むようになります。
大文字小文字は任意の組み合わせのものを認識しますので、`LEFT`や`Right`でも認識します。
...
`<!-- PageFeed -->`のようなHTMLコメント行を挿入することで、強制的に改ページすることができます。

単語間のスペースと大文字小文字は任意の組み合わせのものを認識しますので、`Page feed`でも、`pagefeed`や`pageFeed`でも認識します。
また、省略形として`pf`や`PF`等で記述することもできます。
